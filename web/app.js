...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: GTFS ZTP Kraków · WST Wieliczka (kiedyPrzyjedzie.pl) · SKA — Koleje Małopolskie (gtfs.kasznia.net)' }));
  ...
  function applyFilters() {
    ...
    // the trains carry metro=1, so one toggle can hold the trams and another
    // the three SKA lines — the split Berlin's legend needed.
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      if (!(opts && opts.attrib === false)) {
        ...
        const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: ZTP Kraków · WST Wieliczka · SKA (Koleje Małopolskie)';
